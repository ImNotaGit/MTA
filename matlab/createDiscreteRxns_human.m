...
% stat==0: for a pair of sample; stat==1: for two groups of samples, each group with multiple samples, in which case source and target are matrices where the columns are samples, and smean and tmean are parameters for determining -1/0/1
...
if stat ==1
    ...
    for i = 1:length(model.genes_unique)
        if length(find(model.rxnGeneMat(:,i)))>10 % if too many reactions mapped to a gene, then ignore this gene # always check manually!!!! and manually check everything befpre running
            ignoreG(i)=1;
        end
    end
