...
function[score,stat] = MTActrl(model, v_ref, discrete_rxns_vector, solver)
% no reaction KO, as a control
...
switch solver
    ...
    case 'cplex'
        Res = RunCplexMIQP(model,0);
    case 'gurobi'
        Res = RunGurobiMIQP(model,0);
end

stat = Res.result_status;
